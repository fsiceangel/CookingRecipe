...
const RecipeDetail: React.FC<RecipeDetailProps> = ({ recipe, onBack, uiText }) => {
  return (
    <div className="bg-white/80 dark:bg-slate-800/60 p-6 sm:p-8 rounded-2xl border border-sky-200 dark:border-slate-700 animate-fade-in">
      <button
        onClick={onBack}
        ...
        <div className="lg:col-span-1">
          ...
          <ul className="space-y-2">
            {recipe.ingredients.map((ingredient, index) => (
              <li key={index} className="flex justify-between items-center bg-sky-50 dark:bg-slate-700/50 p-3 rounded-md text-sky-800 dark:text-slate-200">
                <span>{ingredient.name}</span>
                {ingredient.amount && <span className="text-sky-600 dark:text-slate-400 font-medium">{ingredient.amount}</span>}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="lg:col-span-2">
            ...
            <ol className="space-y-4">
                ...
                <li key={index} className="flex items-start gap-3">
                    ...
                    <p className="text-sky-900 dark:text-slate-300 leading-relaxed pt-1">{step}</p>
                </li>
                ))}
            </ol>
            ...
            {recipe.notes && recipe.notes.length > 0 && (
                <div className="mt-8">
                    ...
                    <div className="space-y-3 bg-sky-50/50 dark:bg-slate-800/50 p-4 rounded-lg border border-sky-200 dark:border-slate-700">
                    {recipe.notes.map((note, index) => (
                        <p key={index} className="text-sky-800 dark:text-slate-400">{note}</p>
                    ))}
                    </div>
                </div>
            )}
